...
@interface PrayTime : NSObject {
    ...
    /*  this.methodParams[methodNum] = new Array(fa, ms, mv, is, iv);
     ...
     mv : maghrib parameter value (in angle or minutes)             昏礼
     is : isha selector (0 = angle; 1 = minutes after maghrib)      宵礼
...
//---------------------- Julian Date Functions -----------------------      //公历相关函数
-(double) julianDate: (int)year andMonth:(int)month andDay:(int)day;
-(double)calcJD: (int)year andMonth:(int)month andDay:(int)day;

//---------------------- Calculation Functions -----------------------
-(NSMutableArray*)sunPosition: (double) jd;                             //太阳位置
-(double)equationOfTime: (double)jd;                                     //时差
-(double)sunDeclination: (double)jd;                                     //太阳偏角
-(double)computeMidDay: (double) t;                                      //正午
